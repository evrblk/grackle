...
func TestValidateListWaitGroupJobsRequest(t *testing.T) {
	tests := []struct {
		name        string
		request     *gracklepb.ListWaitGroupJobsRequest
		shouldError bool
	}{
		{
			name:        "empty request - should fail due to missing namespace name",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if test.shouldError {
				require.Error(t, ValidateListWaitGroupJobsRequest(test.request))
			} else {
				require.NoError(t, ValidateListWaitGroupJobsRequest(test.request))
			}
		})
	}
}
