...
func TestAcquireWriteLock(t *testing.T) {
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response1.Lock.State)
	require.Equal(t, "process_1", response1.Lock.WriteLockHolder.ProcessId)
	require.EqualValues(t, now.Add(time.Hour).UnixNano(), response1.Lock.WriteLockHolder.ExpiresAt)
	require.EqualValues(t, now.UnixNano(), response1.Lock.WriteLockHolder.LockedAt)
...
func TestAcquireReadLock(t *testing.T) {
	...
	require.True(t, response1.Success)
	require.EqualValues(t, now.UnixNano(), response1.Lock.LockedAt)
	require.Equal(t, corepb.LockState_READ_LOCKED, response1.Lock.State)
	require.Len(t, response1.Lock.ReadLockHolders, 1)
	require.Equal(t, "process_1", response1.Lock.ReadLockHolders[0].ProcessId)
	require.EqualValues(t, now.Add(time.Hour).UnixNano(), response1.Lock.ReadLockHolders[0].ExpiresAt)
	require.EqualValues(t, now.UnixNano(), response1.Lock.ReadLockHolders[0].LockedAt)
...
func TestAcquireWriteLockRepeatedly(t *testing.T) {
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response2.Lock.State)
	require.EqualValues(t, now.Add(time.Minute).Add(time.Hour).UnixNano(), response2.Lock.WriteLockHolder.ExpiresAt)
}
...
func TestAcquireReadLockRepeatedly(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, response2.Success)
	require.Equal(t, corepb.LockState_READ_LOCKED, response2.Lock.State)
	require.EqualValues(t, now.Add(time.Minute).Add(time.Hour).UnixNano(), response2.Lock.ReadLockHolders[0].ExpiresAt)
}
...
func TestAcquireLockWriteLockedByAnotherProcess(t *testing.T) {
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response2.Lock.State)
	require.Equal(t, "process_1", response2.Lock.WriteLockHolder.ProcessId)
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response3.Lock.State)
	require.Equal(t, "process_1", response3.Lock.WriteLockHolder.ProcessId)
}
...
func TestAcquireLockReadLockedByAnotherProcess(t *testing.T) {
	...
	require.False(t, response2.Success)
	require.Equal(t, corepb.LockState_READ_LOCKED, response2.Lock.State)
	require.Equal(t, "process_1", response2.Lock.ReadLockHolders[0].ProcessId)
	...
	require.Equal(t, corepb.LockState_READ_LOCKED, response3.Lock.State)
	require.Len(t, response3.Lock.ReadLockHolders, 2)
	require.Equal(t, "process_1", response3.Lock.ReadLockHolders[0].ProcessId)
	require.Equal(t, "process_2", response3.Lock.ReadLockHolders[1].ProcessId)
}
...
func TestDeleteAcquiredLock(t *testing.T) {
	...
	require.True(t, response3.Success)
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response3.Lock.State)
	require.Equal(t, "process_2", response3.Lock.WriteLockHolder.ProcessId)
}
...
func TestReleaseReadLock(t *testing.T) {
	...
	require.NotNil(t, response3.Lock)
	require.Equal(t, corepb.LockState_READ_LOCKED, response3.Lock.State)
	require.Equal(t, "process_2", response3.Lock.ReadLockHolders[0].ProcessId)
	...
	require.Len(t, response4.Lock.ReadLockHolders, 0)
}
...
func TestSnapshotAndRestoreLocks(t *testing.T) {
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response4.Lock.State)
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response5.Lock.State)
	...
	require.Equal(t, corepb.LockState_READ_LOCKED, response7.Lock.State)
	...
	require.NoError(t, err)
	require.Equal(t, corepb.LockState_READ_LOCKED, response8.Lock.State)
	require.Len(t, response8.Lock.ReadLockHolders, 1)
	require.Equal(t, "process_2", response8.Lock.ReadLockHolders[0].ProcessId)
}
...
func TestListLocksWithActiveLocks(t *testing.T) {
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, lockMap["lock_1"].State)
	require.Equal(t, corepb.LockState_READ_LOCKED, lockMap["lock_2"].State)
	require.Equal(t, corepb.LockState_READ_LOCKED, lockMap["lock_3"].State)
}
...
func TestListLocksWithExpiredLocks(t *testing.T) {
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response3.Locks[0].State)
}
...
func TestListLocksWithMultipleNamespaces(t *testing.T) {
	...
	require.Equal(t, "namespace_1", response3.Locks[0].Id.NamespaceName)
	...
	require.Equal(t, "namespace_2", response4.Locks[0].Id.NamespaceName)
...
func TestListLocksWithMixedLockStates(t *testing.T) {
	...
	require.Equal(t, corepb.LockState_WRITE_LOCKED, writeLock.State)
	require.NotNil(t, writeLock.WriteLockHolder)
	require.Equal(t, "process_1", writeLock.WriteLockHolder.ProcessId)
	require.Nil(t, writeLock.ReadLockHolders)

	// Check single read lock
	readLockSingle := lockMap["read_lock_single"]
	require.Equal(t, corepb.LockState_READ_LOCKED, readLockSingle.State)
	require.Nil(t, readLockSingle.WriteLockHolder)
	require.Len(t, readLockSingle.ReadLockHolders, 1)
	require.Equal(t, "process_2", readLockSingle.ReadLockHolders[0].ProcessId)

	// Check multiple read lock
	readLockMultiple := lockMap["read_lock_multiple"]
	require.Equal(t, corepb.LockState_READ_LOCKED, readLockMultiple.State)
	require.Nil(t, readLockMultiple.WriteLockHolder)
	require.Len(t, readLockMultiple.ReadLockHolders, 2)
	require.Equal(t, "process_3", readLockMultiple.ReadLockHolders[0].ProcessId)
	require.Equal(t, "process_4", readLockMultiple.ReadLockHolders[1].ProcessId)
}
...
func TestAcquireLockMaxNumberOfLocksPerNamespace(t *testing.T) {
	...
	for i := 0; i < int(maxLocksPerNamespace); i++ {
		...
		require.Equal(t, corepb.LockState_READ_LOCKED, response.Lock.State)
	}
	...
	require.NotNil(t, response.Lock)
	require.True(t, response.Success)
	require.Equal(t, corepb.LockState_WRITE_LOCKED, response.Lock.State)
...
func TestRunLocksGarbageCollectionWithDeletedNamespace(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, getResponse.Lock)
	require.Equal(t, corepb.LockState_WRITE_LOCKED, getResponse.Lock.State)
}
...
func TestGetLockReadLockedWithMultipleHoldersBetweenExpirations(t *testing.T) {
	...
	require.True(t, response2.Success)
	require.Equal(t, corepb.LockState_READ_LOCKED, response2.Lock.State)
	require.Len(t, response2.Lock.ReadLockHolders, 2)
	require.Equal(t, "process_1", response2.Lock.ReadLockHolders[0].ProcessId)
	require.Equal(t, "process_2", response2.Lock.ReadLockHolders[1].ProcessId)
	...
	require.True(t, response3.Success)
	require.Equal(t, corepb.LockState_READ_LOCKED, response3.Lock.State)
	require.Len(t, response3.Lock.ReadLockHolders, 3)
	require.Equal(t, "process_1", response3.Lock.ReadLockHolders[0].ProcessId)
	require.Equal(t, "process_2", response3.Lock.ReadLockHolders[1].ProcessId)
	require.Equal(t, "process_3", response3.Lock.ReadLockHolders[2].ProcessId)
	...
	require.NoError(t, err)
	require.NotNil(t, response4.Lock)
	require.Equal(t, corepb.LockState_READ_LOCKED, response4.Lock.State)
	require.Len(t, response4.Lock.ReadLockHolders, 2) // Only process_1 and process_3 should remain
	...
	require.Len(t, response5.Lock.ReadLockHolders, 0)
...
func TestRunLocksGarbageCollectionWithMultipleExpiringLocks(t *testing.T) {
	...
	for i := 5; i < 10; i++ {
		...
		if response.Lock.State == corepb.LockState_READ_LOCKED {
			require.Len(t, response.Lock.ReadLockHolders, 1, "Lock %d should have exactly one holder remaining", i)
			require.Equal(t, fmt.Sprintf("process_%d_second", i), response.Lock.ReadLockHolders[0].ProcessId)
		}
	}
	...
	for i := 10; i < numLocks; i++ {
		...
		if response.Lock.State == corepb.LockState_READ_LOCKED {
			require.Len(t, response.Lock.ReadLockHolders, 2, "Lock %d should have both holders remaining", i)
